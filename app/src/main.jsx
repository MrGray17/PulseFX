...
function App() {
  ...
  const [radioMode, setRadioMode] = useState('popular');
  ...
  const refreshRadio = async (query = radioQuery) => {
    ...
    setRadioLoading(true); setRadioError(''); setRadioMode('search');
  ...
  const browseRadio = async (mode, countryCode = '') => {
    if (radioLoading) return;
    setRadioLoading(true); setRadioError(''); setRadioMode(mode);
    try { setRadioStations(await pulsefxApi.browseRadio(mode, countryCode)); }
    catch (error) { setRadioStations([]); setRadioError(error instanceof Error ? error.message : String(error)); }
    finally { setRadioLoading(false); }
  };
  const loadRadioCountries = async () => {
    if (radioCountries.length > 0) return;
    try {
      const countries = await pulsefxApi.listRadioCountries();
      const safe = Array.isArray(countries) ? countries.filter((country) => /^[A-Z]{2}$/.test(country?.code ?? '')).slice(0, 250) : [];
      setRadioCountries(safe);
      if (!radioCountryCode && safe.length > 0) setRadioCountryCode(safe[0].code);
    } catch (error) { setRadioError(error instanceof Error ? error.message : String(error)); }
  };
  const openRadio = async () => {
    setActiveTab('radio');
    if (!localCountryCode) {
      try { setLocalCountryCode(await pulsefxApi.getLocaleCountryCode()); } catch {}
    }
    loadRadioCountries();
    if (radioStations.length === 0 && !radioLoading) browseRadio('popular');
  };
  const selectRadioCountry = (countryCode) => {
    setRadioCountryCode(countryCode);
    if (countryCode) browseRadio('country', countryCode);
  };
  ...
  return <main className="app-shell">
    ...
    <section className={enabled ? 'workspace' : 'workspace disabled'}>
      ...
      <div className="content">
        ...
        {activeTab === 'player' && <section className="utility-card media-card"><div className="utility-heading"><div><p className="kicker">AUDIO PLAYER</p><h1>Your music</h1><p>Play local audio through the same PulseFX system-wide enhancement pipeline and organize tracks into persistent playlists.</p></div><div className="preset-row"><button className="refresh-button" onClick={() => pulsefxApi.openDefaultApps()}>Windows Default Apps</button><button className="large-toggle on" onClick={addAudioFiles}><Plus size={17}/>Add audio</button></div></div><div className="playlist-tools"><div className="playlist-tabs">{playlists.map((playlist) => <button key={playlist.id} className={playlist.id === activePlaylistId ? 'playlist-tab active' : 'playlist-tab'} onClick={() => selectPlaylist(playlist.id)}>{playlist.name}<span>{playlist.tracks.length}</span></button>)}</div><div className="playlist-create"><input aria-label="New playlist name" placeholder="New playlist" value={newPlaylistName} onChange={(event) => setNewPlaylistName(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') createPlaylist(); }}/><button aria-label="Create playlist" onClick={createPlaylist}><Plus size={15}/></button><button aria-label="Delete active playlist" disabled={playlists.length <= 1} onClick={deleteActivePlaylist}><Trash2 size={15}/></button></div></div><div className="track-list">{!activePlaylist?.tracks?.length && <div className="empty-state">This playlist is empty. Add MP3, WAV, FLAC, M4A, AAC, OGG, Opus, or WebM audio.</div>}{activePlaylist?.tracks?.map((track, index) => <div className={mediaKind === 'local' && trackIndex === index ? 'track-row active' : 'track-row'} key={track.id}><button className="track-main" onClick={() => selectTrack(index)}><span className="track-number">{mediaKind === 'local' && trackIndex === index && isPlaying ? <AudioLines size={14}/> : index + 1}</span><strong>{track.name}</strong></button><button aria-label={`Remove ${track.name}`} className="track-remove" onClick={() => removeTrack(index)}><Trash2 size={14}/></button></div>)}</div></section>}

        {activeTab === 'radio' && <section className="utility-card media-card"><div className="utility-heading"><div><p className="kicker">INTERNET RADIO</p><h1>Live stations</h1><p>Browse popular, local, or country stations from the open Radio Browser directory and play them through PulseFX.</p></div></div><div className="preset-row"><button className={radioMode === 'popular' ? 'preset active' : 'preset'} onClick={() => browseRadio('popular')} disabled={radioLoading}>Popular</button><button className={radioMode === 'local' ? 'preset active' : 'preset'} onClick={() => browseRadio('local')} disabled={radioLoading}>Local{localCountryCode ? ` · ${localCountryCode}` : ''}</button><label className="output-pill"><Globe2 size={14}/><span><small>COUNTRY</small>{radioCountryCode || 'Choose'}</span><ChevronDown size={14}/><select aria-label="Radio country" value={radioCountryCode} onChange={(event) => selectRadioCountry(event.target.value)}><option value="">Choose country</option>{radioCountries.map((country) => <option key={country.code} value={country.code}>{country.code} · {country.stationcount.toLocaleString()} stations</option>)}</select></label></div><div className="radio-search"><Search size={17}/><input aria-label="Search radio stations" placeholder="Search station name…" value={radioQuery} onChange={(event) => setRadioQuery(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') refreshRadio(); }}/><button onClick={() => refreshRadio()} disabled={radioLoading}>{radioLoading ? 'Loading…' : 'Search'}</button></div>{radioError && <div className="profile-error"><CircleAlert size={15}/><span>{radioError}</span></div>}<div className="station-list">{radioLoading && radioStations.length === 0 && <div className="empty-state">Finding live stations…</div>}{!radioLoading && radioStations.length === 0 && !radioError && <div className="empty-state">No stations loaded.</div>}{radioStations.map((station) => <button key={station.stationuuid} className={activeStation?.stationuuid === station.stationuuid ? 'station-row active' : 'station-row'} onClick={() => playStation(station)}><div className="station-play">{activeStation?.stationuuid === station.stationuuid && isPlaying ? <AudioLines size={16}/> : <Play size={14}/>}</div><div><strong>{station.name}</strong><span>{[station.country || station.countrycode, station.tags?.split(',')[0], station.codec, station.bitrate ? `${station.bitrate} kbps` : ''].filter(Boolean).join(' · ')}</span></div></button>)}</div><p className="muted-note">Station discovery uses the free/open Radio Browser network and automatically fails over between its API servers.</p></section>}
