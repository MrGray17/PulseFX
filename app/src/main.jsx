...
function App() {
  ...
  const addAudioFiles = async () => {
    ...
    setPlaylists((current) => current.map((playlist) => {
      ...
      const additions = files.filter((track) => !known.has(track.id));
      firstNewIndex = playlist.tracks.length;
      return { ...playlist, tracks: [...playlist.tracks, ...additions] };
    }));
    if ((activePlaylist?.tracks?.length ?? 0) === 0 && firstNewIndex >= 0) { setTrackIndex(firstNewIndex); setMediaKind('local'); setActiveStation(null); }
  };
  ...
  useEffect(() => pulsefxApi.onQuickAction((message) => {
    ...
    <section className={enabled ? 'workspace' : 'workspace disabled'}>
      ...
      <div className="content">
        {activeTab === 'enhance' && <section className="enhancer-card">
          ...
          <div className="focus-zone"><div className="focus-copy"><p className="kicker">{effectEnabled[activeEffect] ? 'ACTIVE EFFECT' : 'EFFECT OFF'}</p><h1>{active.label}</h1><p>{active.description}</p></div><div className="dial-wrap"><div className="dial-halo" style={{ '--intensity': `${intensity * 3.6}deg` }}><div className="dial"><ActiveIcon size={34}/><strong>{intensity}%</strong><span>Intensity</span></div></div><input aria-label={`${active.label} intensity`} className="dial-range" type="range" min="0" max="100" value={intensity} onChange={(event) => updateEffectAmount(event.target.value)}/></div><div className="quick-controls"><label><span><small>PREAMP</small><strong>{preamp > 0 ? '+' : ''}{preamp.toFixed(1)} dB</strong></span><input type="range" min="-12" max="9" step="0.5" value={preamp} onChange={(event) => changePreamp(event.target.value)}/></label><label><span><small>PITCH</small><strong>{pitch > 0 ? '+' : ''}{pitch.toFixed(1)} st</strong></span><input aria-label="Pitch semitones" type="range" min="-5" max="5" step="0.1" value={pitch} onChange={(event) => changePitch(event.target.value)}/></label><button className={headphoneEq ? 'headphone-toggle on' : 'headphone-toggle'} onClick={toggleHeadphoneEq}><Headphones size={17}/><span><small>HEADPHONE EQ</small><strong>{headphoneEq ? selectedHeadphone?.name || 'Enabled' : 'Off'}</strong></span><i/></button></div></div>
        </section>}
